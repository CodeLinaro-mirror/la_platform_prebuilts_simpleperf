...
def print_cmd(args: List[str]):
    cmd_str: str = ' '.join(args[:2])
    simpleperf_cmd_str: str = ' '.join(args[2:])
    print(f"* run command: {cmd_str} '{simpleperf_cmd_str}'")
...
class Symbol:
    ...
    def __init__(self, name: str, comm: str, overhead: float, children_overhead: float):
        self.name = name
        self.comm = comm
    ...
    def set_call_tree(self, call_tree: CallTreeNode):
...
class SymbolOverheadRequirement:

    def __init__(self, symbol_name: Optional[str] = None, comm: Optional[str] = None,
                 min_overhead: Optional[float] = None,
    ...
    def is_match(self, symbol: Symbol) -> bool:
        if self.symbol_name is not None:
    ...
    def check_overhead(self, overhead: float) -> bool:
        if self.min_overhead is not None:
...
class SymbolRelationRequirement:

    def __init__(self, symbol_name: str, comm: Optional[str] = None):
        self.symbol_name = symbol_name
        self.comm = comm
        self.children: List[SymbolRelationRequirement] = []

    def add_child(self, child: SymbolRelationRequirement):
        self.children.append(child)
    ...
    def check_relation(self, call_tree: Optional[CallTreeNode]) -> bool:
        if not call_tree:
            return False
...
class Test:
    ...
    def __init__(
            ...
            executable_name: str,
            disable_host: bool,
            record_options: List[str],
            report_options: List[str],
            symbol_overhead_requirements: List[SymbolOverheadRequirement],
            symbol_children_overhead_requirements: List[SymbolOverheadRequirement],
            symbol_relation_requirements: List[SymbolRelationRequirement]):
...
def load_config_file(config_file: str) -> List[Test]:
    tests = []
...
def load_symbol_relation_requirement(symbol_item: ET.Element) -> SymbolRelationRequirement:
    symbol_name = symbol_item.attrib['name']
    comm = None
    if 'comm' in symbol_item.attrib:
        comm = symbol_item.attrib['comm']
...
class Runner:
    ...
    def __init__(self, target: str, perf_path: str, verbose: bool = False):
        self.target = target
    ...
    def record(self, test_executable_name: str, record_file: str, additional_options: List[str] = []):
    ...
    def report(self, record_file: str, report_file: str, additional_options: List[str] = []):
    ...
    def _call(self, args: List[str], output_file: Optional[str] = None):
        pass
...
class HostRunner(Runner):
    ...
    def __init__(self, target: str, verbose: bool = False):
        ...
        super(HostRunner, self).__init__(target, perf_path, verbose)
...
class DeviceRunner(Runner):
    ...
    def __init__(self, target: str, verbose: bool = False):
        ...
        perf_path = 'simpleperf32' if target.endswith('32') else 'simpleperf'
        super(DeviceRunner, self).__init__(target, self.tmpdir + perf_path, verbose)
        self._download(os.environ['OUT'] + '/system/bin/' + perf_path, self.tmpdir)
    ...
    def _call(self, args: List[str], output_file: Optional[str] = None):
        ...
        if self.verbose:
            print_cmd(args_with_adb)
        subprocess.check_call(args_with_adb, stdout=output_fh)
        if output_fh is not None:
            output_fh.close()

    def _download(self, file: str, to_dir: str):
...
class ReportAnalyzer:
    ...
    def _read_report_file(self, report_file: str, has_callgraph: bool) -> List[Symbol]:
    ...
    def _parse_report_items(self, lines: List[str], has_callgraph: bool) -> List[Symbol]:
        symbols: List[Symbol] = []
        cur_symbol: Optional[Symbol] = None
        call_tree_stack: Dict[int, CallTreeNode] = {}
        vertical_columns: List[int] = []
        last_node: Optional[CallTreeNode] = None
        last_depth: int = -1
    ...
    def check_report_file(self, test: Test, report_file: str, has_callgraph: bool) -> bool:
    ...
    def _check_symbol_overhead_requirements(self, test: Test, symbols: List[Symbol]) -> bool:
        result = True
        matched = [False] * len(test.symbol_overhead_requirements)
        matched_overhead = [0.0] * len(test.symbol_overhead_requirements)
    ...
    def _check_symbol_children_overhead_requirements(self, test: Test, symbols: List[Symbol]) -> bool:
        result = True
    ...
    def _check_symbol_relation_requirements(self, test: Test, symbols: List[Symbol]) -> bool:
        result = True
...
def build_runner(target: str, use_callgraph: bool, sampler: str, verbose: bool = False) -> Optional[Runner]:
    if target == 'host32' and use_callgraph:
        print("Current 64bit linux host doesn't support `simpleperf32 record -g`")
        return None
    if target.startswith('host'):
        runner: Runner = HostRunner(target, verbose)
    else:
        runner = DeviceRunner(target, verbose)
    runner.use_callgraph = use_callgraph
    runner.sampler = sampler
    return runner


def test_with_runner(runner: Runner, tests: List[Test]):
    ...
    for test in tests:
        ...
        msg = 'test %s on %s ' % (test.test_name, runner.target)
        if runner.use_callgraph:
            msg += 'with call graph '
        msg += 'using %s ' % runner.sampler
...
def runtest(target_options: List[str], use_callgraph_options: List[bool],
            sampler_options: List[str], selected_tests: Optional[Dict[str, bool]],
            verbose: bool = False):
    ...
    for target in target_options:
        for use_callgraph in use_callgraph_options:
            for sampler in sampler_options:
                runner = build_runner(target, use_callgraph, sampler, verbose)
...
def main():
    parser = argparse.ArgumentParser(
        description='Simpleperf runtest runner.',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  Run all tests on host:
    python3 runtest.py --host

  Run specific tests on device with callgraph:
    python3 runtest.py --device --callgraph --test one_function,two_functions
""")
    parser.add_argument('--host', dest='target', action='store_const', const='host',
                        help='Run tests on host')
    parser.add_argument('--device', dest='target', action='store_const', const='device',
                        help='Run tests on device')
    parser.add_argument('--normal', dest='callgraph', action='store_const', const='normal',
                        help='Run tests without callgraphs')
    parser.add_argument('--callgraph', dest='callgraph', action='store_const', const='callgraph',
                        help='Run tests with callgraphs')
    parser.add_argument('--test', action='append',
                        help='Test name(s) to run (comma-separated, can be repeated)')
    parser.add_argument('--32bit', dest='bit32', action='store_true',
                        help='Run tests on 32-bit targets')
    parser.add_argument('--64bit', dest='bit64', action='store_true',
                        help='Run tests on 64-bit targets')
    parser.add_argument('-v', '--verbose', action='store_true', help='Print commands being run')
    ...
    if args.bit32 or args.bit64:
        target_options = [x for x in target_options if (args.bit32 and x.endswith('32')) or
                          (args.bit64 and x.endswith('64'))]

    use_callgraph_options = [False, True]
    if args.callgraph == 'normal':
        use_callgraph_options = [False]
    elif args.callgraph == 'callgraph':
    ...
    if args.test:
        selected_tests = {}
        for test_arg in args.test:
            for test in test_arg.split(','):
                selected_tests[test] = True

    runtest(target_options, use_callgraph_options, sampler_options, selected_tests, args.verbose)
