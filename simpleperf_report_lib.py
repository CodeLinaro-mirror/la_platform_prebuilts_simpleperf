...
from simpleperf_utils import (bytes_to_str, get_host_binary_path, is_windows, log_exit,
                              str_to_bytes, Log, ReportLibOptions)
...
class ReportLib(object):
    ...
    def __init__(self, native_lib_path: Optional[str] = None):
        ...
        if Log.initialized:
            level: int = Log.logger.getEffectiveLevel()
            self.SetLogSeverity(Log.get_level_name(level))
    ...
    def SetLogSeverity(self, log_level: str):
...
class ProtoFileReportLib:
    ...
    def GetCurrentSample(self) -> Optional[ProtoSample]:
        ...
        ip = sample.callchain[0].vaddr_in_file if sample.callchain else 0
        return ProtoSample(
            ip=ip, pid=thread.process_id, tid=thread.thread_id, thread_comm=thread.thread_name,
    ...
    def GetCallChainOfCurrentSample(self) -> ProtoCallChain:
        ...
        for node in sample.callchain[1:]:
            ...
            ip = node.vaddr_in_file
            entries.append(ProtoCallChainEntry(ip=ip, symbol=symbol))
